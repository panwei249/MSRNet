...
# 尝试从 mne-connectivity 或 mne 中导入 plot_connectivity_circle
HAS_MNE = False
plot_connectivity_circle = None

try:
    # 如果你安装了 mne-connectivity，优先走这里
    from mne_connectivity.viz import plot_connectivity_circle
    HAS_MNE = True
    print("[INFO] 使用 mne_connectivity.viz.plot_connectivity_circle")
except Exception as e1:
    print("[WARN] 从 mne_connectivity.viz 导入失败:", e1)
    try:
        # 经典写法：老版本 mne
        from mne.viz import plot_connectivity_circle
        HAS_MNE = True
        print("[INFO] 使用 mne.viz.plot_connectivity_circle")
    except Exception as e2:
        print("[ERROR] 无法导入 plot_connectivity_circle:", e2)
        HAS_MNE = False
        plot_connectivity_circle = None
...
class Trainer(object):
    ...
    def train(self, data_and_label):
        ...
        for epoch in range(train_epoch):
            ...
            print(best_val_acc)
            try:
                self.visualize_connectivity_circle(model, train_loader)
            except Exception as e:
                logger.error(f"绘制圆形连接图失败: {e}")
    ...
    def visualize_connectivity_circle(self, model, data_loader):
        """
        使用 encoder.get_adj 返回的 adj_recon，基于整个数据集
        计算平均的加权邻接矩阵，并画成 MNE 风格的圆形脑连接圈图。

        - 外圈：固定渐变（每个通道固定颜色），所有图一致；
        - 内圈：线条颜色深浅代表连接强度，右侧保留 colorbar。
        """
        logger = logging.getLogger("train")

        if not HAS_MNE or plot_connectivity_circle is None:
            logger.warning("未成功导入 plot_connectivity_circle，暂无法绘制 MNE 风格圈图。")
            return
        ...
        with torch.no_grad():
            for x, _ in data_loader:
                x = x.to(device)
                # encoder.get_adj 返回 adj_recon: [B, C, C]
                vq_loss, adj_recon, usage, codebook = model.encoder.get_adj(x)

                if adj_sum is None:
                    num_chan = adj_recon.size(1)
                    adj_sum = torch.zeros(num_chan, num_chan, device=device)

                adj_sum += adj_recon.sum(dim=0)
                total_samples += adj_recon.size(0)

        if adj_sum is None or total_samples == 0:
            logger.warning("可视化阶段没有拿到任何 adj_recon，跳过圈图绘制。")
            return

        # ---------- 平均邻接矩阵 ----------
        avg_adj = adj_sum / float(total_samples)
        avg_adj = (avg_adj + avg_adj.t()) / 2.0  # 对称化
        avg_adj = avg_adj - torch.diag(torch.diag(avg_adj))  # 去对角线

        conn = avg_adj.detach().cpu().numpy()
        conn[conn < 0] = 0.0
        if not np.any(conn > 0):
            logger.warning("平均邻接矩阵全为 0，跳过圈图绘制。")
            return

        # 颜色刻度基于真实权值，这样 colorbar 显示是真实数值
        pos_vals = conn[conn > 0]
        vmin, vmax = float(pos_vals.min()), float(pos_vals.max())

        # ---------- 通道名 ----------
        if num_chan == 62 and len(ESI_NEUROSCAN_62) == 62:
            ch_names = ESI_NEUROSCAN_62
        else:
            ch_names = [f"Ch{i + 1}" for i in range(num_chan)]

        # ---------- 外圈颜色：固定渐变（所有图一致） ----------
        # 使用 Spectral_r：黄→橙→红→紫→蓝→绿，风格接近原文
        ring_cmap = plt.colormaps.get_cmap('Spectral_r')
        idx_norm = np.linspace(0.0, 1.0, num_chan, endpoint=False)
        node_colors = ring_cmap(idx_norm)

        # ---------- 内部连线：深浅表示强度 ----------
        # ★ 这里改用 Reds 偏深的部分（0.3~1.0），并提高透明度，弱连接也能看清
        base = plt.colormaps.get_cmap('Reds')(np.linspace(0.3, 1.0, 256))
        base[:, 3] = 0.9  # 统一透明度为 0.9，颜色更实一些
        line_cmap = colors.ListedColormap(base)

        n_lines = 10  # 只画最强的若干条边
        logger.info("开始绘制 MNE 风格圆形脑连接圈图……")
        ...
        plot_connectivity_circle(
            ...
            node_names=ch_names,
            n_lines=n_lines,
            node_colors=node_colors,  # 外圈彩色块（渐变）
            title=None,
            fig=fig,
            subplot=111,
            show=False,
        ...
        save_path = os.path.join(
            self.args.log_dir,
            f"{self.subject_name}_adj_circle.png"
        )
        fig.savefig(
            save_path,
            dpi=400,
        ...
        plt.close(fig)
        logger.info(f"✅ MNE 风格圆形连接图已保存到: {os.path.abspath(save_path)}")
    ...
    #
    #     # ---------- 内部连线：深浅表示强度 ----------
    #     # 基于 Reds，低值接近白色，高值为深红；alpha 统一为 0.8
    #     base = plt.colormaps.get_cmap('Reds')(np.linspace(0.0, 1.0, 256))
    #     base[:, 3] = 0.8  # 统一透明度
    #     base[:60, :3] = 1.0  # 最弱的一段调成几乎白色
    #     line_cmap = colors.ListedColormap(base)
    #
    #     n_lines = 10  # 只画最强的若干条边
    #     logger.info("开始绘制 MNE 风格圆形脑连接圈图……")
    ...
    #         node_edgecolor='black',
    #
    #         linewidth=0.15,  # 连线再细一点
    #         node_linewidth=1.8,
    #
    #         fontsize_title=10,
    #         fontsize_names=7,
    #         fontsize_colorbar=8,
    #         padding=5.0,  # 圈更“张开”一点
    #     )
    #
    #     save_path = os.path.join(
    #         self.args.log_dir,
    ...
    #             x = x.to(device)
    #
    #             # 直接调用 encoder.get_adj，拿到 adj_recon
    #             vq_loss, adj_recon, usage, codebook = model.encoder.get_adj(x)
    #             # adj_recon: [B, C, C]
    #
    #             if adj_sum is None:
    ...
    #                 adj_sum = torch.zeros(num_chan, num_chan, device=device)
    #
    #             adj_sum += adj_recon.sum(dim=0)  # 对 batch 求和
    #             total_samples += adj_recon.size(0)
    #
    #     if adj_sum is None or total_samples == 0:
    #         logger.warning("可视化阶段没有拿到任何 adj_recon，跳过圈图绘制。")
    ...
    #
    #     # 对称化 + 去对角线
    #     avg_adj = (avg_adj + avg_adj.t()) / 2.0
    #     avg_adj = avg_adj - torch.diag(torch.diag(avg_adj))
    #
    #     # 转 numpy
    #     avg_adj = avg_adj.detach().cpu().numpy()
    #
    #     # 如果有负数，截断到 0
    #     avg_adj[avg_adj < 0] = 0.0
    #
    #     if not np.any(avg_adj > 0):
    #         logger.warning("平均邻接矩阵全为 0，跳过圈图绘制。")
    #         return
    #
    #     # 归一化到 [0, 1]
    #     w_pos = avg_adj[avg_adj > 0]
    #     w_min, w_max = w_pos.min(), avg_adj.max()
    #     if w_max > w_min:
    #         con = (avg_adj - w_min) / (w_max - w_min)
    #     else:
    #         con = avg_adj / (w_max + 1e-8)
    #
    #     # ===== 通道名：62 通道 ESI NeuroScan =====
    #     if num_chan == 62 and len(ESI_NEUROSCAN_62) == 62:
    #         ch_names = ESI_NEUROSCAN_62
    #     else:
    #         ch_names = [f"Ch{i + 1}" for i in range(num_chan)]
    #     # ======================================
    #
    #     # 节点强度，用来给外圈着色（和示例中彩色 ring 类似）
    #     node_strength = con.sum(axis=0)
    #     node_strength = (node_strength - node_strength.min()) / \
    #                     (node_strength.max() - node_strength.min() + 1e-8)
    #     node_colors = plt.get_cmap('viridis')(node_strength)
    #
    #     # 只画最强的若干条边，类似示例效果
    #     n_lines = 10  # 你可以自己改成 10 / 15 看看
    #
    #     logger.info("开始绘制 MNE 风格圆形脑连接圈图……")
    #
    #     # 比之前大一点的画布，留出更多空间给标签
    ...
    #
    #     # 关键：调小字体、改白底、去掉花里胡哨的粗线
    #     plot_connectivity_circle(
    #         con,
    #         node_names=ch_names,
    #         n_lines=n_lines,
    #         node_colors=node_colors,      # 外圈彩色块
    #         title=None,
    #         fig=fig,
    #         subplot=111,
    #         show=False,
    #         colorbar=False,              # 通常论文图不需要右边那个 colorbar
    #         colormap='Reds',             # 内部连线用红色渐变
    #         vmin=con[con > 0].min(),
    #         vmax=con.max(),
    #         facecolor='white',
    #         textcolor='black',
    ...
    #         linewidth=1.0,               # 连线更细
    #         fontsize_title=10,
    #         fontsize_names=5,            # 通道标签小一点，避免严重重叠
    #         fontsize_colorbar=8,
    #         padding=8.0,                 # 加大 padding，拉开标签和圆的距离
    #         node_linewidth=1.5,
    #     )
    #
    #     save_path = os.path.join(
